...
export const Search = () => {
  ...
  const [country, setCountry] = useState(searchParams.get('country') || 'UK');
  const [city, setCity] = useState(searchParams.get('city') || '');
  const [deadline, setDeadline] = useState(searchParams.get('deadline') || '');
  const [priceRange, setPriceRange] = useState<number[]>([10, 20]);
  ...
  useEffect(() => {
    const fetchProviders = async () => {
      ...
      try {
        ...
        providerData = providerData.filter(p => p.hourlyRate >= priceRange[0] && p.hourlyRate <= priceRange[1]);
    ...
    fetchProviders();
  }, [category, location, city, priceRange, country]);
        ...
        <Grid size={{ xs: 12, md: 3 }}>
          <Stack spacing={3}>
            <Paper sx={{ p: 3, borderRadius: 4, border: '1px solid', borderColor: 'divider' }}>
              ...
              <List sx={{ p: 0 }}>
                {['All', 'Plumbing', 'Electrical', 'Cleaning', 'Carpentry', 'Painting & Decorating', 'Gardening', 'Handyman', 'HVAC', 'Locksmith', 'Tiling'].map((cat) => (
            ...
            <Paper sx={{ p: 3, borderRadius: 4, border: '1px solid', borderColor: 'divider' }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
                <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary' }}>£10/hr</Typography>
                <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary' }}>£20/hr</Typography>
              </Box>
            </Paper>
          </Stack>
        </Grid>
